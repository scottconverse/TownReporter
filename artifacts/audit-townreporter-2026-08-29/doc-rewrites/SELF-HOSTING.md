<!--
AUDIT DRAFT — 2026-08-29, Technical Writer. Replacement for SELF-HOSTING.md.
Changes from the shipped file, and nothing else:
  * Re-framed as a worked example of one machine, not generic setup (DOC-007)
  * Each section labelled portable / this machine only
  * The `npm test` paragraph corrected against a measured run (DOC-002)
  * "Everything below is running and was verified end to end" attributed to
    the maintainer rather than stated flatly, because an auditor cannot
    re-verify production from a dev checkout
Everything else is preserved verbatim. It is good, hard-won detail.
Confirm before shipping: the production facts (tunnel, DNS, email routing,
scheduled-task state) are the maintainer's attestations. The auditor did not
and could not check them — townreporter.org was out of scope.
-->

# TownReporter — one machine, serving a live paper

**This is a worked example, not the setup guide.** Portable deployment
instructions — any OS, any host — are in [docs/setup.md](docs/setup.md). What
follows is how *this particular* Windows machine serves
**https://townreporter.org**: a Node process behind a Cloudflare Tunnel, no
hosting bill, no ports open on the router. Read it for the reasoning; copy the
parts that match your setup.

Each section is marked:

- **Portable** — applies wherever you run TownReporter.
- **This machine** — specific to one Windows box, one Cloudflare account, one
  operator profile. The reasoning may still be useful; the commands will not
  be.

Everything below was running and verified end to end by the maintainer at the
time of writing.
...
*This machine, but the shape is portable.*
...
*This machine (Windows Task Scheduler).*
...
*This machine — but the lesson is portable: pick a port nothing else owns.*
...
*Portable. The `__Host-` cookie rule below bites every deployment.*
...
`npm test` is deterministic, offline and free. Measured on this machine with
the CLI installed and signed in and no override set: **534 tests, 531 passing,
3 skipped, 0 failing, 17.2 seconds**, and no provider contacted. That is
enforced rather than assumed — `scripts/newsroom-security.test.mjs` fails the
build if any test in the default suite can reach a live model unasked.

The live model evaluation is opt-in and separate:

```bash
RUN_LIVE_MODEL_TESTS=1 npm run test:live-model
```

(Setting `TOWNREPORTER_CLAUDE_CODE=0` is no longer needed to keep a test run
free, and it costs you the Opinion desk, which requires the CLI by design.)

---

## Email

*This machine's Cloudflare account.*
...
*Portable — the endpoint and the fail-closed behaviour are the app's.*
