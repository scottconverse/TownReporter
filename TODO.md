...
- [~] **0.6.25 release candidate:** configurable sections and manual page watching are implemented; sensitive regressions and local editor/reader walkthroughs passed. Final exact-commit CI and GitHub publication remain release gates. Promotion requires Halo.
...
5. [~] **Legal removal — candidate implemented; independent review and release verification pending.** Distinct from normal 30-day trash. Owner policy of 2026-09-06: a sealed, owner-only copy of removed text expires automatically after 12 months; an explicit court order to destroy means keep nothing. Audit and backup handling must respect that exception. Track affected backups and incomplete operator cleanup honestly; ordinary Delete is not this feature. The candidate includes owner preview/case records, calendar expiry, no-retention destruction and stale filing/restore guards. Known captured/shared copies block destruction and require local-operator review; external backup erasure is not claimed. See [the owner workflow](docs/editor.md#legal-removal-owner-workflow).
6. [~] **Investigative settings — implemented; integrated release gates pending.** Editable search window (default 90 days; a search preference, not guaranteed date filtering) and verify-per-round limit (default 6), with a visible “verified X of N” summary.
