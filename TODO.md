...
6. [~] **Investigative settings — implemented in the next candidate; independent review and release gates pending.** Editable search window (default 90 days; a search preference, not guaranteed date filtering) and verify-per-round limit (default 6), with a visible “verified X of N” summary.
