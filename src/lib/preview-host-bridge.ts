...
export function isSafeBridgePath(path: string): boolean {
  ...
  if (/\.zip$/i.test(path) || path.startsWith("/auth/popup")) {
    return false;
  }
  try {
...
export function installPreviewHostBridge(
  ...
  const ancestorOrigin =
    typeof location.ancestorOrigins !== "undefined" && location.ancestorOrigins.length > 0
      ...
      : null;
  let parentOrigin = resolveParentEmbedderOrigin(
    false,
    document.referrer,
    ancestorOrigin,
  ...
  const onMessage = (event: MessageEvent) => {
    ...
    if (
      !parentOrigin &&
      isAllowedPreviewParent(event.origin, window.location.hostname)
    ) {
...
export function collectRoutePathsFromTree(routeTree: unknown): string[] {
  ...
  const walk = (node: unknown) => {
    ...
    if (full !== null && full !== "") {
      ...
      if (/\.zip$/i.test(path) || path === "/get-the-code" || path.startsWith("/auth/popup")) return;
      paths.add(path);
    } else if (full === "") {
      paths.add("/");
    }
