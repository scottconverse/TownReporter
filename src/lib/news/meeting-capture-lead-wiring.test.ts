...
/*
  ...
  `storage_root` the SQL stub returns is unrelated; it is the archive that leaks.
...
before(() => {
  ...
  process.env.TOWNREPORTER_DATA_ROOT = mkdtempSync(join(tmpdir(), "townreporter-lead-wiring-root-"));
});
