...
export function isAllowedPreviewParent(origin: string, guestHostname = ""): boolean {
  try {
    const url = new URL(origin.includes("://") ? origin : `https://${origin}`);
    if (url.protocol !== "https:" && url.protocol !== "http:") return false;
    if (isGrokEmbedderOrigin(url.origin)) return true;
    if (
...
export function resolveParentEmbedderOrigin(
  ...
  for (const candidate of [referrer, ancestorOrigin ?? ""].filter(Boolean)) {
    try {
      ...
      if (isAllowedPreviewParent(url.origin, guestHostname)) return url.origin;
    } catch {
      // try next candidate
    }
  }
