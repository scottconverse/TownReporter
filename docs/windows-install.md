...
The repository includes automated Windows installer and package checks. The [0.6.52 release guide](releases/0.6.52.md) says the original 0.6.50 packaged-install job failed and does not assert a fresh 0.6.52 Windows package result, a published ZIP, or human acceptance. No fresh-machine human acceptance result is documented here.

## Download and install

1. Open [TownReporter 0.6.52](releases/0.6.52.md) for the release's limits and evidence boundaries. For an actual download, use the [latest published release](https://github.com/scottconverse/TownReporter/releases/latest). Download the Windows x64 asset named `TownReporter-<version>-windows-x64.zip` if that release provides it (for this guide, `TownReporter-0.6.52-windows-x64.zip`); do not use the source-code ZIP. Extract the ZIP completely into a folder you intend to keep. Do not run it from inside the ZIP. If the Windows asset is missing, stop and use a release that provides it.
