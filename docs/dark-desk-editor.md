...
As of **0.4.3** the hop engine is the same as 0.3.0, hops belong to the file, and a claim is `resolved` only when the quoted evidence is in the captured document (an empty excerpt stays unresolved). Keep digging is a persisted job; Start digging no longer dumps the last 16 snapshots into the file. This is the signed-in investigative desk at `/desk/dark`. It is **not** the public paper and **not** the reporting queue.
