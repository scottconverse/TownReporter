...
**Published ZIP SHA-256:** `1e781465600bd6ee7b08953253a55002a748bb84b4f8014827af991d0c490463`, verified against the attached `TownReporter-0.6.60-windows-x64.zip.sha256` sidecar and GitHub asset digest.
**Hash rule:** The sidecar is the authority for the published artifact. This note records the prior v0.6.60 asset's hash in the current source history; the v0.6.60 ZIP itself was not modified. A release note inside a ZIP must not embed that same ZIP's hash, because doing so changes the bytes being hashed.
