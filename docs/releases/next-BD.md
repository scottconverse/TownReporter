...
3. **Section tags** come from `newsroom_sections` and are used on the lead, the grid and the article page; the opinion tag is the one yellow-filled tag, with fixed `#111` text (yellow is a fill, never a text color).
...
- `scripts/contrast-audit.mjs` audits the **desk's** tokens only — a grep of it for the reader's tokens returns nothing — so it says nothing about the paper's own palette. The paper's contrast rests on the rule that yellow is a fill and never a text color, and on the measured 14 px floor.
