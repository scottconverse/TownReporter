...
Current version: [0.6.69](0.6.69.md). Full detail: [CHANGELOG](../../CHANGELOG.md).

### Recent releases

- **0.6.69** — Stops the scan throwing work away, and fixes the machine upkeep found while watching the paper. A story the scanner meets twice in one run becomes one lead with both source addresses kept; a strong match against a live lead is still held as a possible duplicate; and a strong match against a KILLED lead now depends on the facts — a new finding carrying a concrete date, dollar amount or number the killed lead lacked is filed in Held as **"Developing: new facts on a story you killed"**, linked to the old lead and showing the old kill reason, while a reworded why is not a new fact and a killed lead whose facts are unchanged is still folded into the old one with the scan summary saying so. An index, section or RSS feed page no longer counts as "the same URL" as an article. The NEW row replaces **≈ PRINTED** with **"Looks already printed: \<headline\>"** plus a one-press **Kill as duplicate** that records the reason and a link to the piece; the **Held** tab carries a count badge; a killed lead shows **"Came back N times"**; and **Compare** is a real side-by-side of both leads with three presses — **Not a duplicate — move to New**, **Same story — kill this one**, **Newer facts — reopen the old one** — each answering in the editor's words. A killed lead's page shows the lead, when and why it was killed, and a **Reopen** that keeps the record and says the kill was undone. Upkeep: `ops/promote.ps1` and `installer/Install.ps1` hash through `Get-TownReporterFileHash` because `Get-FileHash` is missing from a PowerShell 5.1 session with PowerShell 7's `PSModulePath`; hand-named `.sql`/`.dump` safety copies are carried to `<offsite>\other-safety-copies\` under the same `.partial`-and-verify discipline and never deleted from D:; the test copy on 3100 is started after a reboot by `ops/start-stage.ps1` — start-only, reached by hand, by a **Start the test copy** button and by the five-minute watchdog, never a new scheduled task, and never touching a process that is not ours; staging writes `%LOCALAPPDATA%\TownReporter\staged-copy.json` so the copy is found in whatever checkout staged it, gated on path, port, commit and a real build; and the writing ladder's middle rung runs whichever model LM Studio actually has loaded (native `/api/v0/models`), never an embedding model, with `TOWNREPORTER_QWEN_MODEL` still able to pin one. One additive migration, `0094_lead_kill_record.sql` (four nullable `leads` columns and one index; no scheduled task, service or program). See [the release guide](0.6.69.md); it does not assert GitHub publication, production deployment, or live-model proof.

- **0.6.68** — Makes the backup its own job and gives it a second copy. Until this release the only `pg_dump` call in the tree was inside a promotion, no scheduled task ran one, and the folder beside the app held 61 files and about 12.5 GB with nothing pruned and nothing on the second drive. One library now holds that call, and three callers use it: the nightly run driven by the five-minute watchdog the owner already has (after 2:00 AM, when the newest backup is older than 20 hours, skipped while an editor job runs, never two at once), `ops\backup.ps1` — also the Control page's **Back up now** button — and `promote.ps1` itself. Every local backup not already verified on D: is copied, each copy written as `.partial` and renamed only when its size **and** SHA-256 match, and the local folder is pruned to the newest three only after every local file is proved to be on D:; a missing or unwritable drive, a copy that fails its check, or a truncated local dump each delete nothing and raise an alert, nothing is ever deleted from D:, and a copy that would leave under 100 GB free is refused. A file counts as a backup only if it opened, closed, ended with the promised closing marker and passed a size floor. `ops\lib-alert.ps1` watches six conditions — paper down over 10 minutes, public site down over 10 minutes, the daily scan failed or not run by 8:00 AM when scheduled, the last good backup over 30 hours old, the offsite copy failing, the second drive short of room — firing once when each starts and once when it clears, into the Control page's new **Attention** card, a Windows notification, and a phone push through ntfy that is off until `ALERT_NTFY_TOPIC` is set in `.env`. The Control page also gains **Last backup** and **Copy on D:**, both read from the reports the runs write rather than by enumerating a sleeping drive, and no card is green when its probe did not answer. The watchdog now reads `LastTaskResult` `267009` (`SCHED_S_TASK_RUNNING`) as still running rather than as a failure. No migration, no new scheduled task, no new program. See [the release guide](0.6.68.md); it does not assert GitHub publication, production deployment, or live-model proof.

- **0.6.67** — Gives the editor the three controls a story was missing. A machine-written to-do longer than the desk's own reading bound no longer blocks Save or Publish: the desk clips what it writes at a word boundary, reads to-dos at that same bound, normalises a stored one instead of refusing the round trip, and reports a notes failure as a plain sentence beside a story that prints anyway; raw validation output no longer reaches an editor on any surface. The headline box looks editable, a **Redraft** keeps a headline the editor changed (the model's own attempt is filed beside it, not over it), **Use the lead's headline** and **Suggest headlines** apply only on a click, and a printed story can be re-headed from its story page or the **Published** page — words change, the slug and every link do not, and the old headline, the account and the time go to the additive `article_headline_history` table and the action log. Publish reads **Publish in \<Section\>**, confirms that section for the version being printed in the same press, is disabled with its reason when the scanner placed no section, and the server still refuses a draft whose section nobody confirmed or a carried section that is not the draft's own. Every section chosen by hand is logged and counted on Stats. One additive migration, `0093_editor_headline_control.sql`. See [the release guide](0.6.67.md); it does not assert GitHub publication, production deployment, or live-model proof.

- **0.6.66** — The paper comes back by itself after a reboot: the logon start waits for Postgres to answer a real query rather than trusting an open TCP port, migrations run through `cmd.exe` so a child's stderr can never terminate the script and are retried three times with every attempt logged, and a start that cannot serve the paper exits non-zero — a result the watchdog now reads, running the `\TownReporter` task itself after a three-minute boot grace rather than leaving a failed start for someone to notice. Redlib's install root is now `REDLIB_INSTALL_ROOT` in the app's `.env`, outside AppData, with `ops/redlib-relocate.ps1 -To <dir>` to move an existing install out in one step. See [the release guide](0.6.66.md); it does not assert GitHub publication, production deployment, or live-model proof.
