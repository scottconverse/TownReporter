...
**Current software version: [0.6.70](docs/releases/0.6.70.md).** Corrections do more, YouTube is read the official way, the suggested-source list can be cleared, and a draft is measured before it prints. Opening a correction now shows **What was wrong** and **What is right** with **Suggest wording** — one note in the paper's voice, built only from those two lines and the story, and refused rather than offered when the model returns a fragment — and **Use a plain note**, which writes the standard sentence with no model at all. **Also fix the story text**, off unless the editor turns it on, opens the printed body so one press of **Publish correction** publishes the note and updates the words in one transaction, with the body it replaced kept in an append-only record. YouTube channels are read through Google's YouTube Data API when the owner saves a key on the Server page — **channels.list**, **playlistItems.list** and **videos.list**, never **search.list** — with the key encrypted, write-only and sent in a header, and the channel tab, the public feed and yt-dlp kept as the fallback for no key, a refused key, a dead network or a spent day. **Sources** gains **Suggested sources: N**, newest first, each row carrying why the page was suggested and who found it, with accept, reject, a note and batch presses that commit as one transaction. A news draft gets a **Style check** list measured in code — filler words, dressed-up verbs, unattributed claims, long paragraphs — with one bounded model repair round that never adds a fact and is refused if it changes a quotation, number, name or URL. Three additive migrations. GitHub remains the authority for publication state. [0.6.69 release guide](docs/releases/0.6.69.md) · [Changelog](CHANGELOG.md).

See [the deployment boundary](SELF-HOSTING.md) before diagnosing the live paper.
Release source, package metadata, installation checks and deployment evidence are recorded separately in the [0.6.70 release guide](docs/releases/0.6.70.md).
