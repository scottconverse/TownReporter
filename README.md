...
**Current software version: [0.6.64](docs/releases/0.6.64.md).** The scheduled daily scan can be left on Automatic: it no longer has to name one provider before the schedule can be saved, and it walks the same writing ladder stories, scans and Dark Desk use — DeepSeek v4.1 Flash, then Qwen 3.6 35B on this computer when it is loaded, then Codex Terra. A schedule saved before this release keeps the model it names, and the owner switches it on Scan settings. A scan that fetched no source text now says so instead of reporting that the writing pass returned nothing. GitHub remains the authority for publication state. [0.6.63 release guide](docs/releases/0.6.63.md) · [Changelog](CHANGELOG.md).

See [the deployment boundary](SELF-HOSTING.md) before diagnosing the live paper.
Release source, package metadata, installation checks and deployment evidence are recorded separately in the [0.6.64 release guide](docs/releases/0.6.64.md).
...
**0.6.64** is the current version. The scheduled daily scan can be left on Automatic instead of naming one provider before it can be saved, and it walks DeepSeek v4.1 Flash, then Qwen 3.6 35B on this computer when it is loaded, then Codex Terra. A schedule saved before this release keeps the model it names, and the owner switches it on Scan settings. A scan that fetched no source text now says so instead of reporting that the writing pass returned nothing. Read the [0.6.64 release guide](docs/releases/0.6.64.md) for what it claims and what it does not.
