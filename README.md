...
**Current software version: [0.6.65](docs/releases/0.6.65.md).** The Desktop shortcut for this machine now opens a **Control page** instead of a numbered terminal menu. One line reads **Everything is up** or **N things need attention**, each check is a card with a plain verdict and the one button that fixes it, and the six buttons use the menu's own wording — Check, Restart the paper, Restart the tunnel, Start everything, Stop everything, Restart the Reddit reader. It reports what `ops/status.ps1 -Json` says, answers only on `127.0.0.1:3095`, refuses any button press that is not the page's own, runs one action at a time, and asks in a dialog before it takes the paper offline. Nothing on the page publishes, deletes or edits content, and no new scheduled task is created. The numbered menu is still the fallback. GitHub remains the authority for publication state. [0.6.64 release guide](docs/releases/0.6.64.md) · [Changelog](CHANGELOG.md).

See [the deployment boundary](SELF-HOSTING.md) before diagnosing the live paper.
Release source, package metadata, installation checks and deployment evidence are recorded separately in the [0.6.65 release guide](docs/releases/0.6.65.md).
...
**0.6.65** is the current version. The Desktop shortcut opens a Control page — one line saying whether everything is up, a card per check with a plain verdict and the one button that fixes it, and the six actions under the menu's own wording. It reads `ops/status.ps1 -Json`, answers only on loopback, refuses a button press that is not the page's own, runs one action at a time, and asks before it takes the paper offline. The numbered menu remains the fallback. Read the [0.6.65 release guide](docs/releases/0.6.65.md) for what it claims and what it does not.
