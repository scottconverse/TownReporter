...
**Current software version: [0.6.71](docs/releases/0.6.71.md).** The local model picker says what is loaded. Choosing **Local model** used to list every model on disk with no way to see which were in memory; a model that is already loaded now reads `<id> · loaded` and sorts ahead of the rest inside its server group. **Use whatever is loaded** is the first choice — stored as an ordinary pick rather than a new column, so no migration — and it is also the new default when a local server reports a loaded non-embedding model and nothing has been stored. It resolves at call time through the same function the Automatic rung already used, LM Studio before Ollama, and the receipt names the model that actually ran. **The desk never loads a model**: a hand-picked local model that the server reports as not loaded stops before the call with *"`<id>` is not loaded in LM Studio. Load it there, or pick Use whatever is loaded."*, and **Use whatever is loaded** with nothing loaded stops with *"Local model: nothing is loaded in LM Studio or Ollama. Load a model there, or pick a model."* — the desk's own sentence, reaching the operator as the guidance line. A server that reports no load state (llama.cpp) behaves exactly as before, and an Ollama Cloud pick is never blocked. GitHub remains the authority for publication state. [0.6.70 release guide](docs/releases/0.6.70.md) · [Changelog](CHANGELOG.md).

See [the deployment boundary](SELF-HOSTING.md) before diagnosing the live paper.
Release source, package metadata, installation checks and deployment evidence are recorded separately in the [0.6.71 release guide](docs/releases/0.6.71.md).
