...
**Current software version: [0.6.66](docs/releases/0.6.66.md).** The editor's controls. A long to-do the desk wrote itself no longer stops a story being saved or printed — the desk clips its own writing at a word boundary, reads to-dos back at the same bound, and normalises a stored one rather than refusing it, so no editor ever meets raw validation output. A notes failure is a plain sentence beside a story that is on the paper anyway. The headline box looks editable, a **Redraft** no longer replaces a headline an editor changed, and **Use the lead's headline** and **Suggest headlines** sit beside it. A printed story can be re-headed on its story page and on the Published page — the words change, the URL does not — with the old headline, the account and the time kept in a new append-only history. And the publish button now reads **Publish in \<Section\>** and confirms that section in the same press, refusing a draft whose section nobody chose; a section that differed from the scanner's is logged and counted on Stats. One additive migration. GitHub remains the authority for publication state. [0.6.65 release guide](docs/releases/0.6.65.md) · [Changelog](CHANGELOG.md).
...
**0.6.66** is the current version. It gives the editor the three controls that were missing from a story: a to-do the desk wrote too long no longer blocks Save or Publish, a headline an editor changed survives a redraft and can be rewritten even after the story has printed — same URL, with the old words kept — and Publish now reads **Publish in \<Section\>** and confirms that section in the same press. One additive migration. Read the [0.6.66 release guide](docs/releases/0.6.66.md) for what it claims and what it does not.
