...
**Current software version: [0.6.66](docs/releases/0.6.66.md).** The paper now comes back by itself after a reboot. On 2026-09-25 the logon task wrote its start line and then stopped: no migration line, no app, 502 until someone ran the task by hand. The script ran with `$ErrorActionPreference = "Stop"`, and a native command's stderr under `2>&1` arrives in Windows PowerShell 5.1 as a *terminating* error — so when Postgres, still in crash recovery, answered the TCP port before it would accept queries and `migrate` said "the database system is starting up" on stderr, the logon start died on that line. The wait now asks Postgres a real query (`select 1`) for up to three minutes with backoff rather than trusting an open socket; migrations are run with `cmd.exe` doing the redirection, so nothing a child prints can end the script, and they are retried three times with every attempt logged; and if the schema is still not current the start exits non-zero instead of appearing to have succeeded. The watchdog reads that result: after a three-minute boot grace, if the paper is down and `\TownReporter` last ended non-zero, it runs the task itself — never a second copy — instead of leaving a failed start to be noticed by hand. Redlib's install root is now a setting outside AppData (`REDLIB_INSTALL_ROOT`), because an install made from inside the app sandbox is redirected to a path a scheduled task cannot see, and `ops/redlib-relocate.ps1` moves an existing one out in a single step. GitHub remains the authority for publication state. [0.6.65 release guide](docs/releases/0.6.65.md) · [Changelog](CHANGELOG.md).

See [the deployment boundary](SELF-HOSTING.md) before diagnosing the live paper.
Release source, package metadata, installation checks and deployment evidence are recorded separately in the [0.6.66 release guide](docs/releases/0.6.66.md).
