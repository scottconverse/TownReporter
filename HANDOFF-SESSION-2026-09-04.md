> **Historical session record — superseded for current work.** Read [HANDOFF-NEXT-AGENT.md](HANDOFF-NEXT-AGENT.md) and [TODO.md](TODO.md). The “LIVE” and staging statements below are attributed to the Halo-local sessions that wrote them, not new verification by the remote developer. Old hold/queue instructions do not override later owner decisions.
...
**Absence gate ladder — corrected from source on 2026-09-07:** (1) official-domain search; (2) city-wide query without a site restriction; (3) deterministic document-word synonym variation; (4) configured local-press domains. Returned evidence must belong to configured official or local-press domains. The original wording incorrectly described AI-generated synonyms and arbitrary community/social sources.
