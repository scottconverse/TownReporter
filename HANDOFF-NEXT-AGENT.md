...
Active units: a private recorded-findings review panel still needs full browser acceptance; a draft persistence fix requires a current job claim and editor membership and commits draft, lead, audit and job completion together. Its six regressions pass on PGLite and disposable PostgreSQL, with independent mutation proof. A separate raw-JSON truncation fix is under review; its actual-worker regression checks complete evidence through draft saving and publication. Historical damaged rows are not rewritten. These units are not yet merged or released. Reconcile TODO after each green merge; do not imply that automatic publication or the daily operating target is complete.
