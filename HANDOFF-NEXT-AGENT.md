...
The published 0.6.30 manual routine-notice check adds an owner-triggered guarded fetch and bound captured-text review for approved Schema.org Event sources in the library, parks/recreation and community/arts formats. Its parsed candidates remain unverified and it creates no monitor, lead, draft, article or publication. The current beta candidate extends guarded checks to all six approved structured pathways and adds separate paused-by-default automatic editions with deterministic logistics-only eligibility, evidence/authority fences, idempotent ordinary-article publication, correction/review handling, and visible run history. Final beta gates and release remain pending; arbitrary website extraction and semantic verification remain unfinished.
