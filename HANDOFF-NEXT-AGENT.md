...
Active units: PR21 adds the recorded-findings review pane; it is not exhaustive claim coverage or independent fact-checking. Candidate `ce44e65` failed the source-to-reader CI journey; corrected `64d4cfa` passed all 14 application and both Windows jobs (1,761 tests passed, zero failed, 48 explicit skips). Subsequent local review reproduced a delayed-acknowledgement UI failure and inherited newsroom isolation defects. Local corrections keep reporting context/captures in the originating newsroom, restrict public evidence to the public edition, and reject cross-room/cross-URL capture references. Root reproduced the public leak in an anonymous disposable browser and independently falsified four guards; restored reporter/public checks passed 13/13. The combined built browser passed all 28 steps; anonymous public evidence remained readable for the same edition and refused foreign evidence and mixed-room comparisons. Build/typecheck passed, lint has zero errors and 12 baseline warnings, and tampercheck is clean. A new exact-candidate CI run remains required before merging. PR20 is merged as `5261f96`; its candidate passed all 14 application and both Windows jobs (1,742 passed, zero failed, 48 explicit skips). Editor-selected draft batches are prepared in a separate branch: backend-focused 50/50, a sensitive PostgreSQL lock mutation/red-green check, plus final merged-source typecheck/build/lint and offline browser acceptance have passed locally; exact-candidate CI is pending. Automatic publication and measured daily operating targets remain unfinished. Preserve failed-run evidence and do not describe the earlier green candidate as proof of later corrections.
