...
Active units: a private recorded-findings review panel still needs full browser acceptance; draft persistence protection merged in PR18 (`5dcbef1`), with current claim/membership and atomic results plus completion. Candidate `a74e1ef` passed 14 application and two Windows jobs (1,738 passed, zero failed, 48 explicit skips). A separate terminal-status follow-up is under review so late errors cannot label completed work failed. A separate raw-JSON truncation fix is under review; its actual-worker regression checks complete evidence through draft saving and publication. Historical damaged rows are not rewritten. The evidence panel, serialization and terminal-status units are not yet released; resolve their exact PR status before continuing. Reconcile TODO after each green merge; do not imply that automatic publication or the daily operating target is complete.
