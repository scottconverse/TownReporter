...
- Takeover baseline: `cd437a9`, package version **0.6.23**. Resolve current HEAD and CI again before every merge/release; this line records the takeover, not a moving release pointer. Published v0.6.30 is exact source `5e4e1d8cd9a73eb1d25393947ec434752ca56105`; CI `3419199095114` passed 14/14, Windows CI `341919909192` passed 2/2, and published installation verification `34193266163` succeeded.
...
Editor-selected draft batches merged in [PR23](https://github.com/scottconverse/TownReporter/pull/23) as `653e091`; exact implementation candidate `645e2167f1662e12cedad8318d9b944480985e79` passed [14 application jobs](https://github.com/scottconverse/TownReporter/actions/runs/34177659175) and [two Windows jobs](https://github.com/scottconverse/TownReporter/actions/runs/34177659181). Owner-controlled routine-notice permissions merged in [PR25](https://github.com/scottconverse/TownReporter/pull/25) as `2bbb0be`; six-family pure structural contracts merged in [PR26](https://github.com/scottconverse/TownReporter/pull/26) as `1545d41`; mandatory unsaved section review merged in [PR27](https://github.com/scottconverse/TownReporter/pull/27) as `373f91d`; and [PR28](https://github.com/scottconverse/TownReporter/pull/28), merge `40b72bf`, adds bounded saved PrimeGov and Schema.org Event adapters. These units are included in published 0.6.30. The permissions and adapters do not activate, draft or publish notices, structural results remain unverified, and adapters do not yet cover all six families. Automatic publication, broader claim coverage/corroboration and measured daily operating targets remain unfinished. Preserve failed-run evidence and do not describe an earlier green candidate as proof of later corrections.

The published 0.6.30 manual routine-notice check adds an owner-triggered guarded fetch and bound captured-text review for approved Schema.org Event sources in the library, parks/recreation and community/arts formats. Its parsed candidates remain unverified, the other three approved formats report unavailable adapters, and it creates no monitor, lead, draft, article or publication. Explicit eligibility, freshness, conflict, review-handoff, publication-history and idempotent-publication automation remain unfinished.

Deferred local cleanup remains unfinished: seven prototype scripts are removed (467 cumulative text/script artifacts), while 110 PNGs remain blocked by tool policy after individual validation. The owner has been informed; independent feature work proceeds while the blocked cleanup remains recorded.
