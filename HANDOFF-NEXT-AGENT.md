...
Current implementation unit: daily scheduled ordinary scans, isolated branch `codex/daily-scan-automation`. Its first independent review and browser acceptance exposed result-write fencing, malformed-input and repeated-save defects. Those are fixed, with independent sensitive mutations and a passing built-browser settings journey. It remains unmerged pending exact-candidate CI, including real PostgreSQL concurrency; it must not be described as released. Correction newsroom ownership and the strict Reddit elapsed-time fix merged in [PR15](https://github.com/scottconverse/TownReporter/pull/15); its candidate `c10fbc4` passed 14 application CI jobs and both Windows checks with 1,701 tests passed, zero failed and 46 explicit skips. Version 0.6.28 is the bounded corrective release; publication and production deployment remain separate from implementation.
