...
Active units: the recorded-findings workbench panel initially passed a 21-step local browser journey and 21 focused tests. PR21 candidate ce44e65 later failed the source-to-reader CI journey; all other application jobs and both Windows jobs passed. Panel cache refresh, pending-action controls and premature draft-completion detection are corrected locally. Root verification passed 108 focused tests, the full 27-step browser journey, build and typecheck; the separate source-to-reader journey passed seven steps. Job-identity and cache-key mutations reproduced their respective faults. Lint has zero errors and 12 baseline warnings. The corrected exact-candidate CI remains required before merge. It provides private captured-text reading, explicit judgments, stale-save refusal, failed-reload input preservation and honest malformed-record warnings. It is not exhaustive claim coverage or independent fact-checking. PR20 merged as `5261f96`, preserving completed job status after late bookkeeping errors. Candidate `5a7b5b4` passed all 14 application and both Windows jobs: 1,742 tests passed, zero failed, 48 explicit skips. Reconcile actual PR status before continuing. Automatic publication, bounded batch drafting and the daily operating target remain unfinished.
